class OpenViduLayout {
  ...
  positionElement(elem, x, y, width, height, animate) {
    ...
    if (animate) {
      const animationOptions = {
        duration: animate.duration || 200,
        easing: animate.easing || 'swing',
        complete: () => {
          this.fixAspectRatio(elem, width)
          if (animate.complete) {
      ...
      elem.style.transition = `all ${animationOptions.duration}ms ${animationOptions.easing}`
      elem.addEventListener('transitionend', animationOptions.complete, {
        once: true,
      })
  ...
  getCSSNumber(elem, prop) {
    const cssStr = getComputedStyle(elem).getPropertyValue(prop)
  ...
  getHeight(elem) {
    const heightStr = getComputedStyle(elem).getPropertyValue('height')
    return heightStr ? parseInt(heightStr, 10) : 0
  }

  getWidth(elem) {
    const widthStr = getComputedStyle(elem).getPropertyValue('width')
